    ...
    >
        <x-ui.swiper.wrapper>
            @foreach ($items as $key => $item)
                <x-ui.swiper.item key="{{ $key }}" class="relative">
                    <div class="bg-cover bg-no-repeat" style="background-image: url('{{ Storage::url($item['image']) }}')">
                        <div class="container">
        ...
        <div class="flex justify-between absolute right-0 bottom-0 z-40">
            <div
                ...
                @foreach ($items as $key => $item)
                    ...
                    $key++;
                    @endphp
                    <span class='flex leading-120' @click="goToSlide({{ $key }})"
                          :class="{'bullet-active': currentSlide === {{ $key }}}">
                     <span
